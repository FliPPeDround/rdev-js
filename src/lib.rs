...
use napi::{
  bindgen_prelude::Uint32Array,
  threadsafe_function::{ErrorStrategy, ThreadsafeFunction, ThreadsafeFunctionCallMode},
  JsFunction, Result,
};
use rdev::{
  display_size, listen, simulate, Button, Event,
  EventType::{ButtonPress, ButtonRelease, KeyPress, KeyRelease, MouseMove, Wheel},
  Key,
};
...
#[napi(
  js_name = "displaySize",
  ts_return_type = "[width:number, height:number]"
)]
      ...
      let duration_since_epoch = match event.time.duration_since(UNIX_EPOCH) {
        ...
        Err(_) => {
          return Err(napi::Error::from_reason(
            "Failed to convert time".to_owned(),
          ))
        }
      };
